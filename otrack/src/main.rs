...
fn send_request(request: DaemonRequest) -> Result<DaemonResponse> {
    let mut stream = UnixStream::connect(SOCKET_PATH)
        .context("Could not connect to otrackd. Is the daemon running?")?;
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Status => match send_request(DaemonRequest::GetStatus)? {
            DaemonResponse::Status {
                active_app,
                session_start,
                is_focus_mode,
                focus_remaining_secs,
            } => {
                println!("--- Current Status ---");
                println!(
                    "Active App: {}",
                    active_app.unwrap_or_else(|| "None".into())
                );
        ...
        Commands::Report => match send_request(DaemonRequest::GetReport)? {
        ...
        Commands::Start { duration } => {
            send_request(DaemonRequest::StartFocus {
        ...
        Commands::Waybar => {
            if let Ok(DaemonResponse::Report {
                top_apps,
                today_total,
            }) = send_request(DaemonRequest::GetReport)
            {
                let tooltip = top_apps
                    .iter()
...
fn run_dashboard() -> Result<()> {
    ...
    loop {
        terminal.draw(|f| {
            ...
            if let Some(DaemonResponse::Status {
                active_app,
                is_focus_mode,
                focus_remaining_secs,
                ..
            }) = status
            {
                status_text.push(Line::from(vec![
                    Span::raw("App: "),
                    Span::styled(
                        active_app.unwrap_or_default(),
                        Style::default().fg(Color::Cyan).bold(),
                    ),
                    Span::raw(" | Focus: "),
                    if is_focus_mode {
                        Span::styled(
                            format_duration(focus_remaining_secs.unwrap_or(0)),
            ...
            if let Some(DaemonResponse::Report {
                top_apps,
                today_total,
            }) = report
            {
                let list_items: Vec<ListItem> = top_apps
                    .iter()
                    .map(|(app, dur)| ListItem::new(format!("{}: {}", app, format_duration(*dur))))
                    .collect();

                let list =
                    List::new(list_items).block(Block::default().borders(Borders::ALL).title(
                        format!("Top Apps (Total: {})", format_duration(today_total)),
                    ));
